...
<div class="fieldcontain ${hasErrors(bean: episodioInstance, field: 'nome', 'error')} required">
	...
	<g:textField name="nome" maxlength="30" pattern="${episodioInstance.constraints.nome.matches}" required="" value="${episodioInstance?.nome}"/>

</div>

<div class="fieldcontain ${hasErrors(bean: episodioInstance, field: 'temporada', 'error')} required">
	<label for="temporada">
		<g:message code="episodio.temporada.label" default="Temporada" />
		<span class="required-indicator">*</span>
	</label>
	<g:field name="temporada" type="number" min="0" value="${episodioInstance.temporada}" required=""/>
